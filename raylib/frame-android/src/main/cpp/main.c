...
#define LOG_TAG "jolt_raylib_first_frame"
...
typedef int (*first_frame_fn)(void);
...
int main(int argc, char *argv[]) {
  ...
  void *library = dlopen("libjoltraylib-frame.so", RTLD_NOW | RTLD_LOCAL);
  if (!library) {
  ...
  first_frame_fn first_frame =
      (first_frame_fn)lookup("raylib_first_frame");
  LOGI("jolt_lookup raylib_first_frame=%s thread=%d owner=%d",
       first_frame ? "ok" : "missing", thread_id(), owner);
  if (!first_frame) {
    shutdown();
    dlclose(library);
    return 4;
  ...
  LOGI("raylib_first_frame result=%d thread=%d owner=%d", frames, thread_id(),
       owner);
  shutdown();
  LOGI("jolt_library_shutdown thread=%d owner=%d", thread_id(), owner);
  dlclose(library);
  LOGI("first-frame bootstrap complete thread=%d owner=%d", thread_id(), owner);
  return frames == 120 ? 0 : 5;
}
